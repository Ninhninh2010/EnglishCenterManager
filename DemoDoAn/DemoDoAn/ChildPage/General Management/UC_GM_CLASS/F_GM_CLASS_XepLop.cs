            ...
            foreach (CheckBox cbox in pnl_QLNgayHoc.Controls.OfType<CheckBox>())
            {
                if (cbox.Checked == true)
                {
                    ...
                    {   //tìm những "Thứ" và "Ca" đã được chọn đã có lịch dạy của môn khác hay chưa
                        if (dtLichHoc.Rows[r]["Thu"].ToString() == thu && dtLichHoc.Rows[r]["Ca"].ToString() == ((DataRowView)cbb_GioHoc.SelectedItem)["Ca"].ToString())
                        {
                            //nếu có thì phòng/GV đó đã có lịch
                            for (int i = 0; i < dtCheck.Rows.Count; i++)
                            {
                                if (dtCheck.Rows[i][thuocTinhCheck].ToString() == dtLichHoc.Rows[r][thuocTinhCheck].ToString())
                                {
