...
namespace DemoDoAn.ChildPage.HocTap
{
    internal class BangDiemDAO
    {
        ...
        {
            string sqlStr = string.Format("SELECT KHOAHOC.MaKH, KHOAHOC.TenKH, LOPHOC.MaLop, LOPHOC.TenMon, NgayBatDau, NgayKetThuc, SoHocVien, GIANGVIEN.GvID, GIANGVIEN.HOTEN, HocPhi\r\nFROM LOPHOC inner join KHOAHOC on LOPHOC.MaKH = KHOAHOC.MaKH join GIANGVIEN on GIANGVIEN.GvID = LOPHOC.GiangVien \r\nWHERE LOPHOC.MaLop = '{0}'", maLop);
            return dbConn.LayDanhSach(sqlStr);
        }
